...
DIRECTORY_FIGURES = 'C:\Users\susanna\Documents\GitHub\PSA_SFERAII\Modelling\ResultsFigure/' #'C:/Users/Admin/Documents/GitHub/PSA_SFERAII/Modelling/Results/'
...
ModResults = 'C:\Users\susanna\Documents\GitHub\PSA_SFERAII\Modelling\ModelicaResults/' #'C:/Users/Admin/Desktop/SFERA_II_Sim/'
TimeSim = []
TimeExp = []
NN = []
T_su = []
...
lns4 = ax2.plot(TimeExp[3]-StartModTime[3]-ZoomTimeShift[3],DNIExp[3]/max(DNIExp[3]),'o', markeredgewidth=0.05,markerfacecolor='None',markeredgecolor='darkgoldenrod',label = r'DNI')
lns5 = ax2.plot(TimeExp[3]-StartModTime[3]-ZoomTimeShift[3],m_wfExp[3]/max(m_wfExp[3]),'o', markeredgewidth=0.05,markerfacecolor='None',markeredgecolor='black',label = r'$\dot{m}_\mathrm{wf}$')
...
# Plot DNI
ax2 = ax1.twinx()# for an array of equal element
lns4 = ax2.plot(TimeExp[4]-StartModTime[4]-ZoomTimeShift[4],DNIExp[4]/max(DNIExp[4]),'o', markeredgewidth=0.05,markerfacecolor='None',markeredgecolor='darkgoldenrod',label = r'DNI')
lns5 = ax2.plot(TimeExp[4]-StartModTime[4]-ZoomTimeShift[4],m_wfExp[4]/max(m_wfExp[4]),'o', markeredgewidth=0.05,markerfacecolor='None',markeredgecolor='black',label = r'$\dot{m}_\mathrm{wf}$')
plt.ylabel(r'DNI - $\dot{m}_\mathrm{wf}$ [-]')
lns = lns1+lns2+lns3+lns4+lns5
labs = [l.get_label() for l in lns]
ax1.legend(lns,labs,loc = 'upper right',fancybox='True',shadow='True',labelspacing=0.1,fontsize=10,numpoints=1)
...
# Plot DNI
ax2 = ax1.twinx()# for an array of equal element
lns4 = ax2.plot(TimeExp[0]-StartModTime[0]-ZoomTimeShift[0],DNIExp[0]/max(DNIExp[0]),'o', markeredgewidth=0.05,markerfacecolor='None',markeredgecolor='darkgoldenrod',label = r'DNI')
lns5 = ax2.plot(TimeExp[0]-StartModTime[0]-ZoomTimeShift[0],m_wfExp[0]/max(m_wfExp[0]),'o', markeredgewidth=0.05,markerfacecolor='None',markeredgecolor='black',label = r'$\dot{m}_\mathrm{wf}$')
plt.ylabel(r'DNI - $\dot{m}_\mathrm{wf}$ [-]')
lns = lns1+lns2+lns3+lns4+lns5
labs = [l.get_label() for l in lns]
ax1.legend(lns,labs,loc = 'upper right',fancybox='True',shadow='True',labelspacing=0.1,fontsize=10,numpoints=1)
...
if SAVE_FIGURES == 'True': fig.savefig(DIRECTORY_FIGURES+'/'+'_MassFlowChange.pdf',dpi=1000)
plt.show()